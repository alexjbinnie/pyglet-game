...
class Game(metaclass=Singleton):
    ...
    def update(self, dt):
        if Game().Server:
            if self.bullets_dirty:
                print("Clear bullets")
                bullets_destroyed = [bullet for bullet in self.bullets if
                                     not bullet._active and bullet._death_counter <= 0]
    ...
    def random_spawn(self):
        spawns_dist = [(spawn, sum([np.linalg.norm(spawn._position - ship._position) for ship in self.ships])) for spawn
                       in self.spawns]
